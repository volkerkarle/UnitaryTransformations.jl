...
FAST_COEFFS_ONLY = get(ENV, "UT_FAST_COEFFS_ONLY", "0") == "1"
...
t_L0_o2 = @elapsed result_L0_o2 =
    schrieffer_wolff(H, P_L0; order = 2, simplify_mode = :fractions, include_QQ = false)
println("  Order 2 finished in $(round(t_L0_o2, digits=2)) s")

# Full SW with legacy P↔Q-only expansion (include_QQ=false)
t_L0_o4 = @elapsed result_L0_o4 = schrieffer_wolff(
    H,
    P_L0;
    order = 4,
    simplify_mode = :fast,
...
println("  Order 4 finished in $(round(t_L0_o4, digits=2)) s")
...
t_ground_o2 = @elapsed result_ground = schrieffer_wolff(
    H,
    P_ground;
    order = 2,
...
println("  Order 2 finished in $(round(t_ground_o2, digits=2)) s")

t_ground_o4 = @elapsed result_L0n0_o4 = schrieffer_wolff(
    H,
    P_ground;
    order = 4,
    simplify_mode = :fast,
...
println("  Order 4 (diagonal_only) finished in $(round(t_ground_o4, digits=2)) s")
...
"""
    detuning_subs(B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋)

Build substitution dictionaries for detuning patterns.
"""
function detuning_subs(B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋)
    linear_subs = Dict(
    ...
    quadratic_subs = Dict(
        ...
        # 8B² + 2Bω_c - ω_c² = -(ω_c - 4B)(ω_c + 2B) = -Δ₁₂₋ Δ₊
    ...
    return linear_subs, quadratic_subs
end
...
function simplify_with_detunings(expr::QuExpr, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋)
    linear_subs, quadratic_subs = detuning_subs(B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋)
...
function simplify_coeff_with_detunings(coeff, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋)
    coeff === nothing && return nothing
    coeff isa Num || return coeff

    linear_subs, quadratic_subs = detuning_subs(B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋)

    new_coeff = expand(coeff)
    for (pattern, replacement) in quadratic_subs
        new_coeff = Symbolics.substitute(new_coeff, pattern => replacement)
    end
    for (pattern, replacement) in linear_subs
    ...
    new_coeff = simplify_fractions(new_coeff)
    return clean_rationals(new_coeff)
end

function simplify_hermitian_coeffs_with_detunings(
    coeffs::NamedTuple,
    B,
    ω_c,
    Δ₊,
    ...
    return (
        E0 = simplify_coeff_with_detunings(coeffs.E0, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋),
        A = simplify_coeff_with_detunings(coeffs.A, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋),
        Ω = simplify_coeff_with_detunings(coeffs.Ω, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋),
        κ = simplify_coeff_with_detunings(coeffs.κ, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋),
        μ = simplify_coeff_with_detunings(coeffs.μ, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋),
        ν = simplify_coeff_with_detunings(coeffs.ν, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋),
...
if FAST_COEFFS_ONLY
    println("Skipping full H_P simplification for Option B (set UT_FAST_COEFFS_ONLY=0 to enable).")
else
    t_L0_simplify = @elapsed H_P_L0_simplified =
        simplify_with_detunings(result_L0_o4.H_P, B, ω_c, Δ₊, Δ₋, Δ₁₂₊, Δ₁₂₋)
    println("Order 4 H_P (simplified) in $(round(t_L0_simplify, digits=2)) s:")
...
if FAST_COEFFS_ONLY
    println("Option B (L=0): skipped full simplification (see coefficients below)")
else
...
"""
function extract_hermitian_form(
    H_eff::QuExpr;
    ...
    # Quadratic operators
    coeff_a2 = extract_coefficient(H_eff, a()^2; simplify_coeff = simplify_coeff)           # a²
    coeff_adag2 = extract_coefficient(H_eff, a'()^2; simplify_coeff = simplify_coeff)       # a†²
    coeff_adaga = extract_coefficient(H_eff, a'() * a(); simplify_coeff = simplify_coeff)     # a†a (number operator)

    # Quartic operators
    coeff_a4 = extract_coefficient(H_eff, a()^4; simplify_coeff = simplify_coeff)                    # a⁴
    coeff_adag4 = extract_coefficient(H_eff, a'()^4; simplify_coeff = simplify_coeff)                # a†⁴
    coeff_adag3a = extract_coefficient(H_eff, a'()^3 * a(); simplify_coeff = simplify_coeff)           # a†³a
    coeff_adaga3 = extract_coefficient(H_eff, a'() * a()^3; simplify_coeff = simplify_coeff)           # a†a³
    coeff_adag2a2 = extract_coefficient(H_eff, a'()^2 * a()^2; simplify_coeff = simplify_coeff)        # a†²a²
    ...
    function numeric_zero(expr)
        subs_list = [
            Dict(B => 1.0, ω_c => 2.3, g => 0.1, c₀₁ => 0.6, c₁₂ => 0.4),
            Dict(B => 0.8, ω_c => 1.7, g => 0.2, c₀₁ => 0.5, c₁₂ => 0.3),
        ]
...
if FAST_COEFFS_ONLY
    println("From coefficient-only detuning simplification:")
    t_L0_coeffs = @elapsed begin
        coeffs_L0_raw = extract_hermitian_form(
            result_L0_o4.H_P;
            simplify_coeff = false,
            check_hermitian = false,
        )
        coeffs_L0_simplified = simplify_hermitian_coeffs_with_detunings(
            coeffs_L0_raw,
            B,
            ω_c,
            Δ₊,
    ...
    println("  Coefficient extraction in $(round(t_L0_coeffs, digits=2)) s")
    print_hermitian_form(coeffs_L0_simplified; name = "H_eff")
else
    println("From simplified H_P (with detunings):")
    t_L0_coeffs = @elapsed coeffs_L0_simplified = extract_hermitian_form(H_P_L0_simplified)
    println("  Coefficient extraction in $(round(t_L0_coeffs, digits=2)) s")
    print_hermitian_form(coeffs_L0_simplified; name = "H_eff")
end
...
if FAST_COEFFS_ONLY
    coeffs_L0_o2_raw = extract_hermitian_form(
        result_L0_o2.H_P;
        simplify_coeff = false,
        check_hermitian = false,
    )
    coeffs_L0_o2 = simplify_hermitian_coeffs_with_detunings(
        coeffs_L0_o2_raw,
        B,
        ω_c,
        Δ₊,
    ...
    t_L0_o2_coeffs = @elapsed coeffs_L0_o2 = extract_hermitian_form(H_P_L0_o2_simplified)
    println("  Order 2 coefficient extraction in $(round(t_L0_o2_coeffs, digits=2)) s")
    print_hermitian_form(coeffs_L0_o2; name = "H_eff (order 2)")
end
