...
using QuantumAlgebra: QuExpr, QuTerm, BaseOperator, BaseOpProduct, TLSCreate_, TLSDestroy_, normal_form, comm
...
function project_to_subspace(H::QuExpr, P::Subspace)
    ...
    for constraint in P.constraints
        ...
        result = substitute_operator_eigenvalue(result, constraint.operator, constraint.eigenvalue)
    end
...
function substitute_spin_projection(H::QuExpr, spin_name, spin_inds, pm_eigenvalue)
    ...
    for (term, coeff) in H.terms
        ...
        while i <= length(ops)
            ...
            if i < length(ops) && 
               ops[i].t == TLSCreate_ && ops[i+1].t == TLSDestroy_ &&
               ops[i].name == spin_name && ops[i].inds == spin_inds &&
               ops[i+1].name == spin_name && ops[i+1].inds == spin_inds
        ...
        if isempty(new_ops)
            ...
            new_term = QuTerm(term.nsuminds, term.δs, term.params, 
                             term.expvals, term.corrs, BaseOpProduct())
        else
            new_term = QuTerm(term.nsuminds, term.δs, term.params,
                             term.expvals, term.corrs, BaseOpProduct(new_ops))
        end
